...
### <span class="star-badge">⭐</span> RabbitMQ
...
1. 生产端丢失：开启生产者确认机制（ACK），消息发送失败自动重试。
2. MQ 端丢失（MQ 宕机）：开启消息持久化+集群部署。
3. 消费端丢失：开启手动 ACK，处理完业务逻辑，再手动 ACK 确认消息。

**重复消费**

- **原因**：网络重试、消费者宕机重启，同一条消息会被投递多次
- **解决方案**：核心：保证消费者接口幂等性。
  - 唯一 ID 去重：消息带唯一ID，消费前查数据库，已消费则直接返回。
  - 数据库唯一约束：插入数据库用唯一键。
  - 分布式锁：消费前加锁，防止重复消费。

**消息积压**

- **原因**：消费者处理速度太慢、消费者数量太少等。
- **解决方案**：
  - 紧急扩容：增加消费者数量。
  - 优化消费逻辑：减少慢SQL、异步处理、批量消费等。
  - 临时转储：积压太多，先把消息转到临时队列，慢慢消费。

**消息顺序性**

- **原因**：MQ 默认多队列、多消费者并发消费，无法保证顺序。
- **解决方案**：
  - 严格顺序：同一业务数据发送到一个队列+单个消费者消费。
  - 分段顺序：按用户ID/订单ID 哈希路由，保证局部有序。
