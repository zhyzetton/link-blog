...
### <span class="star-badge">⭐</span> MySQL 优点
...
### <span class="star-badge">⭐</span> DATETIME 和 TIMESTAMP 的区别
...
### <span class="star-badge">⭐</span> Boolean 类型如何表示

MySQL 中没有专门的布尔类型，而是用 TINYINT(1) 类型来表示布尔值。TINYINT(1) 类型可以存储 0 或 1，分别对应 false 或 true。

### <span class="star-badge">⭐</span> 手机号存储
...
## <span class="star-badge">⭐</span> MySQL 索引
...
主键索引（Primary Key）是 B+ 树结构，用于快速定位数据的索引。
...
### <span class="star-badge">⭐</span>最左前缀匹配原则
...
## <span class="star-badge">⭐</span> MySQL 日志
...
1. ==prepare 阶段==：将数据写入到 redo log，同时将redo log 对应的事务状态设置为 prepare，然后将 redo log 持久化到磁盘。
2. ==commit 阶段==：将数据写入到 bin log，然后将bin log 持久化到磁盘，接着调用引擎的提交事务接口，将 redo log 状态设置为 commit。
...
- Server 层（MySQL 自带）
  ...
  管 SQL 解析、权限、主从复制 -> 只产生 binlog

- InnoDB 引擎（存储核心）
  
  管事务、内存缓冲、数据存储 -> 产生 redo log + undo log

三种日志：

- Binlog 二进制日志
  
  逻辑日志，记录全局操作。作用：1. 主从复制。2. 人为误操作恢复。

  特点：1. 追加写。2. 事务提交后记录。3. 所有引擎通用。

- Redo Log 重做日志

  物理日志，记录数据页修改，宕机恢复

  MySQL 修改数据先改内存，如果宕机，内存的修改全部丢失。

  作用：崩溃恢复。宕机重启后，根据 Redo Log 把内存丢失的修改同步到磁盘。保证了==事务的持久性==，提交了就一定不丢。
...
- Undo Log 回滚日志
  ...
  作用：1. 事务会滚，执行失败/rollback，用Undo Log 恢复成修改前。2. 用于 MVCC 版本链。

## <span class="star-badge">⭐</span> MySQL 事务
...
1. MySQL 锁分层：Server 表锁 + InnoDB 行锁
2. InnoDB 行锁（RR 级别）：
  - 默认：临键锁（记录 + 间隙，防幻读）
  - 唯一索引精准匹配 → 退化为记录锁
  - 唯一索引没匹配到 → 退化为间隙锁
3. 插入：用插入意向锁
4. 无索引：行锁失效 → 锁全表
5. RC 级别：只有记录锁，无临键 / 间隙锁

## <span class="star-badge">⭐</span> 常见SQL 优化手段
